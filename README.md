...
- **파일 준비:** **files 폴더에서** `AccidentHazard_CodeList.xlsx`, `19_24_drunk.csv`, `korea_geojson.json`, `202144079.ipynb` 다운로드 후 Colab 업로드
- (`AccidentHazard_CodeList.xlsx`: 요청변수 코드, `19_24_drunk.csv`: 음주운전사고 CSV, `korea_geojson.json`: 대한민국 행정구역 GeoJSON, `202144079.ipynb`: google colab 실행 파일)
...
- **지도 내 시군구 경계는 해당 시군구의 `위험도 점수`를 나타내고, 원형 마커는 해당 위치의 `사고 건수`와 `사상자 수` 정보를 보여줍니다.**
![시각화이미지1](시각화이미지_1.PNG)
![시각화이미지2](시각화이미지_2.PNG)
![시각화이미지3](시각화이미지_3.PNG)
![시각화이미지4](시각화이미지_4.PNG)
